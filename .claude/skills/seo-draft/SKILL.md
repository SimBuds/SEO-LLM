...
3. Check: `bash scripts/check.sh draft outputs/<slug>/draft.md outputs/<slug>/outline.md <brief.json> draft`. The draft deliberately aims above `word_count` (135%; `DRAFT_FACTOR` overrides), because `/seo-rewrite` cuts it back toward the target.
